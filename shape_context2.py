...
testpath = os.path.join(current_directory, 'Output', '*')
print(testpath)
# Database folder (with one image per sample only)
alphapath = os.path.join(current_directory, 'AlphaCenter')
images = []
labels = []
# index = 0
folders = glob.glob(path)
alphaFolder = glob.glob(alphapath)
...
for f in os.listdir(alphapath):
    fpath = os.path.join(alphapath, f)
    image = cv2.imread(fpath,cv2.IMREAD_GRAYSCALE)
    tempimage = cv2.resize(image, (50, 50))
    revertimage = cv2.bitwise_not(tempimage)
    targetimages.append(image)
    targetlabels.append(f.replace(".png", ""))
# index = index + 1
...
testimageList = images
testimage = images[0]
#testimage = cv2.imread("single.png", 0)
...
def paragraph():
    result = []
    for ti in testimageList:
        mini = 100000000
        candidate_image = 0
        for i in range(len(targetimages)) :
            print("calculate distance with ", str(targetlabels[i]).replace("_low", ""))
            dist = distance(ti, targetimages[i])
            if mini > dist:
                mini = dist
                candidate_image = i
        #print("exact value :{}".format(labels[0]))
        perdict = str(targetlabels[candidate_image].replace("_low", ""))
        print("predict ", perdict)
