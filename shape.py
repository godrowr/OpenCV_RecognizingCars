...
class Point:
    ...
    def cart2logpolar(self):
        '''
            ...
            return (rho, theta)
        '''
        #the gradient moving avg decay factor (learning rate decay over each update)
    ...
    def dist2(self, other):
        """
            Find distance from origin (b/w two objects)
...
for folder in folders:
    ...
    for f in glob.glob(folder+'/*.png'):
        ...
        tempimage = cv2.resize(image, (30, 30))
...
#we have 653 total data images, and 653 labels. Each label corresponds to a particular letter in the training data 0-9 and A-Z.
#In this example index 585 represents letter 'U'. Can adjust to test against other images by passing in an index 0-653. Must do the same for labels
#At line 378 to display the exact value for the predicted value using our testimage.
testimage = images[585]
# test image 77 represents number 2
#It mistakes Z for 2. this is an example of an innacurate result. Since both Z and 2 have similar edges, the algorithm may mistake one for the other.
#Example of a correct result using 585 and 586 testimages, representing Letters 'U' and 'X' respectively.

#As we can see, for the Letter 'U' it returns a correct prediction. For 'X' it also predicts the exact value.

#Our segmentation algorithm encounters problems when segmenting images with non-white backgrounds. When playing around with the threshold it may work, but is not
#very accurate. However, for images like example.png it correctly segments the license plate, but only when the lettering is black and background is white. 

#Output folder is the result of segmenting a license plate and storing it for shape_context's use. Shape context should loop over each image and 
#compare it's distance with a target image used from our training data. However, at times it will incorrectly predict results. this is an example of such a case:
...
for i in range(len(targetimages)) :
    ...
    if mini > dist:
        mini = dist
        candidate_image = i
print("exact value :{}".format(labels[585]))
